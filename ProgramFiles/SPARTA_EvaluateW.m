function [W]=SPARTA_EvaluateW(X,pi,gamma,C,Lambda,d,T,K,m,W,eps_C,reg_param)
...
for k=1:K
    idx=(gamma(k,:)==1);
    for i=1:d
        disc_dist(i)=disc_dist(i)+sum((X(i,idx)-C(i,k)).^2);
    end
    Xb=zeros(d,sum(idx));
    for ind_m=1:m
        for i=1:d
            Xb(i,:)=Lambda(ind_m,i+1,k)*X(i,idx);
        end
        alpha=alpha+Xb*Xb';
        bet=bet+(pi(ind_m,idx)-Lambda(ind_m,1,k))*(Xb)';
    end
end
