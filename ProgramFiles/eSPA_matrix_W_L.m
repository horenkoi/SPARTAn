...
function [L] = eSPA_matrix_W_L(X,pi,C,Lambda,gamma,T,d,m, reg_param, eps_C,W,K,eps_Creg);
	xx=X-C*gamma;
    %xx=sqrt(W)*xx;%xx=xx.*(2*ones(d)-eye(d));
    k=1;
    Entr=0;
    for i=1:d
        for j=(i):d
            Entr=Entr+W(i,j).*log(max(W(i,j),1e-12));
        end
    end
    %Entr=sum(sum(W.*log(max(W,1e-12))));
    L=0;
    for i=1:d
        L=L+W(i,i)*sum(xx(i,:).*xx(i,:)); 
        for j=(i+1):d
        L=L+2*W(i,j)*sum(xx(i,:).*xx(j,:)); 
        end
    end
	% Updating the main functional 
	L = L  * (1/(T*d)) + (eps_C/d) * Entr - reg_param/(T*m) * sum(sum(pi.*(log(max(Lambda*gamma,1e-12)))));
