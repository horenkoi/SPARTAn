...
function [out] = eSPA_matrix_W_Replica_ts(in)    
    ...
    while and(delta_L > eps, i <= MaxIter)  % Stop criterion
        ...
        C_W = W_m*C;
        %[L_3] = eSPA_matrix_W_L(X_W, Pi, C_W, Lambda, gamma, T, d, m, reg_param, eps_C, W, K, eps_Creg)
        ...
        time_W = tic();
        [L_3] = eSPA_matrix_W_L(X, Pi, C, Lambda, gamma, T, d, m, reg_param, eps_C, W, K, eps_Creg)

        % Computation of the W -----------> ask Prof. Horenko: is this an analytical solution?
        [W] = eSPA_matrix_W_EvaluateWRegularize(X, gamma, C, d, T, W, eps_C);
        ...
        [C] = eSPA_EvaluateCRegularize_analytic(X, gamma, K, d, T, W_m); 
        C_W = W_m*C;
